...
export const Create_comment = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        console.log(e);
        res.status(401);
        res.send({ message: "Impossible to recreate and impossible to make, well no comment." });
        return;
    }
}

export const Update_comment = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const Delete_comment = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        console.log(e);
        res.status(401);
        res.send({ message: "It was such glorius comment that even deleting it is impossible." });
        return;
    }
}
