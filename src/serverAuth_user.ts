...
import { createNewUser_user, signIn_user, token_refresh } from './handlers/user';
...
app.post('/auth_user/login', 
    body('username').isString().isLength({ min: 3, max: 30}),
    body('password').isString().isLength({ min: 1 })
,handleError ,signIn_user);
app.post('/auth_user/signup', 
    body('username').isString().isLength({ min: 3, max: 30}),
    body('password').isString().isLength({ min: 1 }),
...
,handleError, createNewUser_user);
